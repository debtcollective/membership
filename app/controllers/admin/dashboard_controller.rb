...
class Admin::DashboardController < AdminController
  ...
  def index
    ...
    subscriptions = Donation.where(status: "succeeded", donation_type: Donation::DONATION_TYPES[:subscription])
    @amount_from_subscriptions = subscriptions.sum(&:amount).to_f

    one_off_donations = Donation.where(status: "succeeded", donation_type: Donation::DONATION_TYPES[:one_off])
